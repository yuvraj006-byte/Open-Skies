...
def generate_green_stats(item_type, item_rarity):

    base_points = random.randint(10, 15)
    ...
    if item_type == "Weapons":
        damage = base_points * 2.0
        armor = base_points * 0.4
        shield = base_points * 0.2
        healing = 0

    elif item_type == "Armor":
        damage = base_points * 0.4
        armor = base_points * 2.0
        shield = base_points * 1.2
        healing = 0

    elif item_type == "Shields":
        damage = base_points * 0.2
        armor = base_points * 1.2
...
def create_green_item():

    item = get_random_green_item()

    if not item:
        print("No Green items found.")
        return None

    stats = generate_green_stats(item["type"], item["rarity"])
...
if __name__ == "__main__":
    ...
    green_item = create_green_item()

    if green_item:
        print_item(green_item)
